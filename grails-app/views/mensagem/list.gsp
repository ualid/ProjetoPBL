            ...
            <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic resize">
				<thead>
					<tr>
                        ...
                        <td>${message(code: 'mensagem.dataMensagem.label', default: 'Data Mensagem')}</td>
					
                        <td>${message(code: 'mensagem.emailDestinatario.label', default: 'Email Destinatario')}</td>
					
						<td><g:message code="mensagem.remetente.label" default="Remetente" /></td>
					
                        <td>${message(code: 'mensagem.status.label', default: 'Status')}</td>
				...
				<g:each in="${mensagemInstanceList}" status="i" var="mensagemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:formatDate date="${mensagemInstance.dataMensagem}" /></td>
					
						<td>${fieldValue(bean: mensagemInstance, field: "emailDestinatario")}</td>
					
						<td>${fieldValue(bean: mensagemInstance, field: "remetente")}</td>
					
						<td><g:formatBoolean boolean="${mensagemInstance.status}" /></td>
